...
class LDA:
    def __init__(self, K, alpha, beta, docs, V, smartinit=True):
        ...
        for m, doc in enumerate(docs):
            self.N += len(doc)
            z_n = []
            for t in doc: #分配词语， 将一篇文档词语随机分配到含有多一位topic的两个矩阵中，我们认为每篇文档词语都是从这两个矩阵中随机抽选的
                if smartinit:
    ...
    def inference(self): #inference 先减 再sampling updating 最后再加上去
        """learning once iteration"""
        for m, doc in enumerate(self.docs):
            z_n = self.z_m_n[m]
            n_m_z = self.n_m_z[m]
            for n, t in enumerate(doc): #n:counter  t:element
    ...
    def worddist(self):
        ...
        return self.n_z_t / self.n_z[:, numpy.newaxis] #new a dimension [10, 12] to [[10], [12]]
...
def main():
    ...
    parser.add_option("-f", dest="filename", help="corpus filename", default='ap.txt')  #use any sample .txt for training
    ...
    if options.filename:
        ...
        corpus = vocabulary.load_corpus(options.corpus)  #here is to load 'corpora/wordnet', install first
